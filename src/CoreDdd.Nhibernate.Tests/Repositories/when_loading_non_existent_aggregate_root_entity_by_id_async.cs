...
[TestFixture]
public class when_loading_non_existent_aggregate_root_entity_by_id_async : BasePersistenceTest
{
    [Test]
    public void entity_is_persisted_and_retrieved()
    {
        var testEntityRepository = new NhibernateRepository<TestEntity>(UnitOfWork);
        var ex = Should.Throw<Exception>(async () => await testEntityRepository.LoadByIdAsync(-1));
